...
void DW1000RangingClass::generalStart() {
	...
	if(DEBUG) {
		// DEBUG monitoring
		Serial1.println("DW1000-arduino");
		...
		if(messageType == BLINK && _type == ANCHOR) {
			...
			if((_networkDevicesNumber == 0) || (myDistantDevice == nullptr)) {
				...
				if (DEBUG) {
					...
					Serial1.print(address[0], HEX);
					Serial1.print(":");
					Serial1.println(address[1], HEX);
					*/
				}
				return;
			}
...
void DW1000RangingClass::computeRangeAsymmetric(DW1000Device* myDistantDevice, DW1000Time* myTOF) {
	...
	/*
	Serial1.print("timePollAckReceived ");myDistantDevice->timePollAckReceived.print();
	Serial1.print("timePollSent ");myDistantDevice->timePollSent.print();
	Serial1.print("round1 "); Serial1.println((long)round1.getTimestamp());
	
	Serial1.print("timePollAckSent ");myDistantDevice->timePollAckSent.print();
	Serial1.print("timePollReceived ");myDistantDevice->timePollReceived.print();
	Serial1.print("reply1 "); Serial1.println((long)reply1.getTimestamp());
	
	Serial1.print("timeRangeReceived ");myDistantDevice->timeRangeReceived.print();
	Serial1.print("timePollAckSent ");myDistantDevice->timePollAckSent.print();
	Serial1.print("round2 "); Serial1.println((long)round2.getTimestamp());
	
	Serial1.print("timeRangeSent ");myDistantDevice->timeRangeSent.print();
	Serial1.print("timePollAckReceived ");myDistantDevice->timePollAckReceived.print();
	Serial1.print("reply2 "); Serial1.println((long)reply2.getTimestamp());
	 */
}
